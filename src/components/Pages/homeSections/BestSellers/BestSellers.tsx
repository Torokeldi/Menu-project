...
const BestSellers: React.FC = () => {
  ...
  return (
    <section id="best" className="container">
      <div className={styles.bestSellersContent}>
        <div className={styles.bestSellersInfo}>
          <div className={styles.logo}>
            <Image src={Line} alt="Decorative line" />
            <span className={styles.bestSellers}>Best Sellers</span>
          </div>
          <h1 className={styles.bestSellersTitle}>
            You Only Reserve <br /> Exception
          </h1>
          <p className={styles.bestSellersTxt}>
            Each location has a menu that is curated just for them. <br />
            See what new at your Cafesio and you will find Cafesio <br />
        ...
        <div className={styles.sliderWrapper}>
          {currentIndex > 0 && (
            <button
              className={`${styles.shevronButton} ${styles.left}`}
              onClick={prevSlide}
            >
          ...
          <div className={styles.sliderWindow}>
            <div className={styles.bestSellersTrack}>
              {images
                .slice(currentIndex, currentIndex + 2)
                .map((src, index) => (
                  <div className={styles.bestSellersImg} key={index}>
                    <Image
                      src={src}
                      width={266}
                      height={281}
          ...
          {currentIndex + 2 < images.length && (
            <button
              className={`${styles.shevronButton} ${styles.right}`}
              onClick={nextSlide}
            >
